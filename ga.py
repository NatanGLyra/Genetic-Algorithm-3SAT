...
import ajustaarquivo as ajuste
...
def calculateFitness(pPopulation, pClauses, pSize):
    '''
    ...
    tam_pop = len(pPopulation)
    if tam_pop>pSize:
        retirar = tam_pop-pSize
        for i in range(0,retirar):
            deletado = min(pPopulation, key=pPopulation.get)
            del pPopulation[deletado]
...
if __name__ == '__main__':
    ...
    #     population = calculateFitness(population, clauses, populationSize)
    ...
    for nome_arquivo in os.listdir(caminho_pasta):
        ...
        if os.path.isfile(caminho_arquivo):  # Verifica se ̩ um arquivo
            clauses, varAmount = readFile(caminho_arquivo)
            ...
            chave = max(population, key=population.get)
            if (float(population[chave]) == 1.0):
                sucessos +=1
